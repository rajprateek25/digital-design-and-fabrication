...
```cpp
 const int PUMP1_PIN = 3;
const int PUMP2_PIN = 4;
...
void loop() {
  ...
  digitalWrite(PUMP2_PIN, HIGH);  // Turn on Pump 2
  delay(5000);                    // Inflate for 5 seconds

  // --- HOLD PHASE ---
  digitalWrite(PUMP1_PIN, LOW);   // Turn off pumps
  digitalWrite(PUMP2_PIN, LOW);
  digitalWrite(VALVE_PIN, LOW);
  delay(3000);                    // Hold pressure for 3 seconds (Check for leaks!)
...
### 📝 Post 2: Re-engineering the Airflow

Initially, the plan was to use both pumps to inflate the pillow simultaneously. But there's a catch: diaphragm pumps only push air forward and cannot act as a passive exhaust. To rapidly deflate the pillow, I re-plumbed the system to separate the pumps' roles.

**The Swapped-Role Pneumatic Flow**

* 
**Active Inflation:** Pump 2 acts as the sole active inflation source, pumping fresh air straight into the pillow.


* 
**Active Deflation Assist:** Pump 1 is repurposed as an active vacuum. Its suction intake is tubed to the normally-open metal-end port of the FA0520E 3-port valve. When the valve is powered, it routes the pillow's escaping air straight into Pump 1, which rapidly draws the air out and expels it to the atmosphere.



---

### 📝 Post 3: Making it Interactive with a PIR Sensor
...
#define PUMP1_PIN 3       // MOSFET 1 (Now Deflation Pump)
#define PUMP2_PIN 4       // MOSFET 2 (Now Inflation Pump)
